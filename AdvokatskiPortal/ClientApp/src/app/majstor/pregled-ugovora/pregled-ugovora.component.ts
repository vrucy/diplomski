...
export class PregledUgovoraComponent implements OnInit {
  displayedColumns: string[] = ['ime', 'prezime', 'opis', 'cena','pocetakRada','zavrsetakRada','poslednjaIzmena','datumPrijema', 'najkasnijiOdgovor' , 'button'];
  ...
  constructor(private majstorService: MajstorService, public dialog: MatDialog) {
    this.initialize();
  }
  ngOnInit() {
  ...
  initialize() {
    this.majstorService.getUgovorsForMajstor().subscribe(res => {
  ...
  submitPopupForm(result) {
    console.log('RESULTAT POPUPA', result)
     this.majstorService.prepravkaSlucajaMajstora(result).subscribe(res => {
     this.initialize() ;
     });
  }
  ...
  redirectToAccept(slucajMajstor) {
    const ids = { majstorId: slucajMajstor.majstorId, slucajId: slucajMajstor.slucaj.id }
    this.majstorService.prihvatanjeSlucajaOdMajstora(ids).subscribe(res => {
      // this.removeAt(slucajMajstor);
    });
  }

  redirectToReject(slucajMajstor) {
    const ids = {majstorId: slucajMajstor.majstorId , slucajId: slucajMajstor.slucaj.id}
      this.majstorService.odbijanjeSlucajaOdMajstora(ids).subscribe((res: any) => {
        console.log(res);
      });
  }
  handleSlucaj(ids) {
