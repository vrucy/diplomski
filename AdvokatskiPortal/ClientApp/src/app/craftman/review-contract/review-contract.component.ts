...
export class ReviewContractComponent implements OnInit {
  ...
  redirectToAccept(caseCraftman) {
    const ids = { craftmanId: caseCraftman.craftmanId, caseId: caseCraftman.case.id }
    this.craftmanService.AcceptCaseFromCraftman(ids).subscribe(res => {
    });
  }

  redirectToReject(caseCraftman) {
    const ids = {craftmanId: caseCraftman.craftmanId , caseId: caseCraftman.case.id}
  ...
  handleButton(element) {
    ...
    switch (element.caseStatusId) {
      case 8 : 
        return 'Time for answer is over.'
        break
      case 4:
